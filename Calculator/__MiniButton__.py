...
class Button(QPushButton):
    def __init__(self, parent: QWidget, text: str, line_edit: QLineEdit):
        ...
        self.clicked.connect(lambda: line_edit.setText(line_edit.text() + ' ' + text))
    ...
    def restyle_border(self, color=QColor):
        self.setStyleSheet(f'''
            color: white; 
            border-radius: 10px; 
            border: {self.width() // 2}px solid rgb({color.red()}, {color.green()}, {color.blue()})
        ''')
        pass
